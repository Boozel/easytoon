...
protected:
  ...
  // 雑用オブジェクト
	...
	void	PasteSelectBuf(bool pasteOnInterface = FALSE);									// 範囲指定バッファを吐き出す
	void	PasteTransformationToSelection(POINT* ptRaster, POINT* ptPixel, BYTE *pPixel, CDC * pDC );
	...
	void CreateDrawingPen(CPen& pen);
...
protected:
	...
	afx_msg void OnTbSelectionTransparency();
	afx_msg void OnUpdateTbSelectionTransparency(CCmdUI* pCmdUI);
	afx_msg void OnText();
	afx_msg void OnUpdateMemoryBtn( CCmdUI* pCmdUI );
	afx_msg void OnChangeBrushPatternFwd();
	afx_msg void OnChangeBrushPatternBwd();
	//}}AFX_MSG
	DECLARE_MESSAGE_MAP()
private:
  ...
  int _QuantizeForCanvas(int pos);
	...
	LOGBRUSH CreateLogbrush(COLORREF color);
	void ClearUndoRedo();
	void ClearSelection();
public:
