...
BEGIN_MESSAGE_MAP(CEasyToonView, CScrollView)
	...
	ON_COMMAND_RANGE(ID_BRUSH_PT0, ID_BRUSH_PT17, OnBrushBtn)
	ON_UPDATE_COMMAND_UI_RANGE(ID_BRUSH_PT0, ID_BRUSH_PT17, OnUpdateBrushBtn)
	...
	ON_COMMAND(ID_TB_SELECT_TRANSPARENCY, OnTbSelectionTransparency)
	ON_UPDATE_COMMAND_UI(ID_TB_SELECT_TRANSPARENCY, OnUpdateTbSelectionTransparency)
	ON_COMMAND(ID_TEXT, OnText)
	ON_WM_KEYDOWN()
	ON_WM_MBUTTONDOWN()
	ON_UPDATE_COMMAND_UI_RANGE(ID_MEM1, ID_MEM4, OnUpdateMemoryBtn)
	ON_COMMAND(ID_CHANGE_BRUSH_PATTERN_FWD, OnChangeBrushPatternFwd)
	ON_COMMAND(ID_CHANGE_BRUSH_PATTERN_BWD, OnChangeBrushPatternBwd)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CEasyToonView::_DrawInterfaceRectangle()
{
	...
	// Pen creation
	CPen pen;
	CBrush brush(Pattern::GetPattern(m_nBrush));
	...
	{
		if(m_crInterfacePenColor==0 && m_nBrush != 17)
		{
			pen.CreatePen(PS_GEOMETRIC , m_nInterfacePenSize, &CreateLogbrush(m_crInterfacePenColor));
		}
		else
		{
			pen.CreatePen( PS_SOLID, m_nInterfacePenSize, RGB(255,255,255));
		}
...
void CEasyToonView::DrawCanvas( CDC* pDC, CRect* pRect )
{
	...
	////Redraw selection depending of selection type
	if((m_iCopyMode == ROTATE) || (m_iCopyMode == FREE))
	{
		_DrawSelectionArea( pDestDC, m_ptCopyDst, 4 );
	}
	else
	{
		_DrawSelectionRect( pDestDC, m_rectCopyDst, 4 );
	}
...
//--------------------------------------------------------------------------
void CEasyToonView::PasteSelectBuf(bool pasteOnInterface)
{
	CDC *pDC = !pasteOnInterface ? GetDocument()->GetFrameDC(m_nCurrentFrame) : &m_dcInterface;
	...
	if( ((m_iCopyMode == ROTATE) || (m_iCopyMode == FREE)) && (m_pbmpCopySrc != NULL) ) 
	{
		...
		if(m_bSelectionTransparent)
		{
			//Use m_pbmpTemp to store the interface bitmap before we draw the transformation content
			m_pbmpTemp = Utils::CopyBitmap(pDC->GetCurrentBitmap());
			
			//Do the transformation and set pixels
			PasteTransformationToSelection(ptRaster,ptPixel,pPixel,pDC);
			
			//Temporarly using m_dcCopySrc to store the old content of the interface
			CBitmap * oldBmp = m_dcCopySrc.SelectObject(m_pbmpTemp);

			//Paste m_dcCopySrc content to the new interface using the SRCAND raster operation 
			//which will give us the transparency
			pDC->BitBlt(0,0,m_sizeFrame.cx, m_sizeFrame.cy, &m_dcCopySrc, 0, 0, SRCAND);
			
			//Restore the old object used by m_dcCopySrc
			m_dcCopySrc.SelectObject(&oldBmp);
			
			//Release temporary bmp
			delete m_pbmpTemp;
		}
		else
		{
	...
	if ( !m_rectCopySrc.IsRectEmpty() && !m_rectCopyDst.IsRectEmpty() )
	{
		...
		pDC->StretchBlt(
			...
			&m_dcCopySrc, rectSrc.left, rectSrc.top, rectSrc.Width(), rectSrc.Height(), srcOpMode );

		//pDC->RestoreDC( -1 );
	}
}
void CEasyToonView::PasteTransformationToSelection(POINT* ptRaster, POINT* ptPixel, BYTE *pPixel, CDC * pDC)
{
	int y,x;
	...
	case pen:
		{
			...
			{
				m_penDraw.CreatePen(PS_GEOMETRIC , nPenSize, &CreateLogbrush(crPen));
			}
			else
			{
			...
			if( nPenSize == 1 ) 
			{
				pdcFrame->SetPixel(ptMouse.x, ptMouse.y,crPen);
			
				//old way to strike point in case of hand drawing bug
				pdcFrame->MoveTo( ptMouse.x, ptMouse.y ); //This line has been uncommented to avoid drawing a line starting at 0,0
	...
	case move:
		{
			...
			if( m_nMoveSubStat == stretchout ) {
				...
				_DrawSelectionRect( pdc, m_rectCopySrc, 4 );
				ReleaseDC( pdc );
			}
			break;
		}
...
void CEasyToonView::_EndCopy()
{
	...
	/*CDC* pdc = GetDC();
	OnPrepareDC( pdc );
	_DrawSelectionArea(pdc,m_ptCopyDst,4);
	_DrawSelectionRect(pdc,m_rectCopyDst,4);
	ReleaseDC( pdc );
	*/
	...
	case pen:
		{
			...
			//pdc->SetPixel(ptMouse.x, ptMouse.y,m_crInterfacePenColor);
	...
	case line:case paraline:case linestrip:
		{
			...
			if (m_nTool==paraline && m_bFirstUsePara==false)
			{
				...
				//Utilisée pour déterminer quelle méthode utiliser pour le							
				//calcule des coordonnées dans KeepDraw
...
void CEasyToonView::OnMemoryBtn(bool Saving, int Pos)
{
	if (!Saving)
	{
		CBitmap * tmpBmp = GetDocument()->GetBitmapFromMemoryList(Pos);
		...
		if(tmpBmp != NULL)
		{
			...
			m_dcCopySrc.SelectObject(tmpBmp);
...
void CEasyToonView::OnChangeBrushPatternFwd()
{
	GetMainFrame()->ChangeBrushPattern(true);
}

void CEasyToonView::OnChangeBrushPatternBwd()
{
	GetMainFrame()->ChangeBrushPattern(false);
}
...
void CEasyToonView::_DrawSelectionRect( CDC *pDC, CRect rect, int nPick)
{
	...
	// ƒyƒ“ì¬
	CPen pen( PS_SOLID, 1, RGB(255,0,0)), *ppenOld;
...
void CEasyToonView::_DrawSelectionArea(CDC *pDC, CPoint pointsrc[], int nPick)
{
	if( (pDC == NULL) ) return;
	CPoint point[4];
	CPoint srcAddedPoints = pointsrc[0] + pointsrc[1] + pointsrc[2] + pointsrc[3];

	if(srcAddedPoints.x == 0 && srcAddedPoints.y == 0)
	{
		return;
	}
...
void CEasyToonView::SetZoomMagnify(int nZoomMagnify)
{
	...
	if (nZoomMagnify!=0 && nZoomMagnify<=FRAMEVIEW_MAXMAGNIFY)
	{
		...
		if (m_nZoomMax==false)
		{
			...
			{
				MousePoint.x=m_sizeFrame.cx;
			}

			if (MousePoint.y>m_sizeFrame.cy)
			{
				MousePoint.y=m_sizeFrame.cy;
			}
	...
	if (nZoomMagnify==FRAMEVIEW_MAXMAGNIFY)
	{
		m_nZoomMax=true;
	}
	else
...
void CEasyToonView::OnTbSelectionTransparency()
{
	m_bSelectionTransparent = !m_bSelectionTransparent;
	
	m_nMoveSubStat = stretchview;

	//Force redraw of selection and canvas
	DrawFrames();
	DrawCanvas(NULL);
}

void CEasyToonView::OnUpdateTbSelectionTransparency(CCmdUI* pCmdUI)
{
	if(m_bSelectionTransparent)
	{
		pCmdUI->SetCheck( 1 );
...
void CEasyToonView::OnText()
{
	...
	m_ptCopyDst[0].x = m_rectCopyDst.left;
	m_ptCopyDst[0].y = m_rectCopyDst.top;
	m_ptCopyDst[1].x = m_rectCopyDst.right;
	m_ptCopyDst[1].y = m_rectCopyDst.top;
	m_ptCopyDst[2].x = m_rectCopyDst.right;
	m_ptCopyDst[2].y = m_rectCopyDst.bottom;
	m_ptCopyDst[3].x = m_rectCopyDst.left;
	m_ptCopyDst[3].y = m_rectCopyDst.bottom;

	DrawFrames(&m_rectCopySrc);
	DrawCanvas(NULL);
}
...
LOGBRUSH CEasyToonView::CreateLogbrush(COLORREF color)
{
	LOGBRUSH logBrush;

	logBrush.lbStyle = BS_PATTERN;
	logBrush.lbColor = color;
	logBrush.lbHatch = (ULONG_PTR)Pattern::GetPattern(m_nBrush)->GetSafeHandle();

	return logBrush;
}

void CEasyToonView::CreateDrawingPen(CPen& pen)
{
...
void CEasyToonView::ClearUndoRedo()
{
	UndoRedoPos=0;
	MaxUndo = 0;
	m_blCanRedo = FALSE;
...
void CEasyToonView::ClearSelection()
{
	m_rectCopySrc.SetRectEmpty();
	m_rectCopyDst.SetRectEmpty();
