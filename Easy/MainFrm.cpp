...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
  ...
  m_wndSelect.SetWindowText("Sélection");
...
Posrect.OffsetRect(450, 0);
  ...
  //-------- ƒc[ƒ‹ƒo[‚ÉƒtƒŒ[ƒ€ƒCƒ“ƒ^[ƒoƒ‹EƒXƒsƒ“‚ð’Ç‰Á
	...
	rect.top += 4;
	m_staticFrameInt.Create ( "Durée", WS_VISIBLE, rect, &m_wndToolBar, IDC_FRAMEINTCAP );
	// iƒGƒfƒBƒbƒgj
...
void CMainFrame::OnUpdateFrameDefaultDelayIndicator(CCmdUI* pCmdUI)
{
	...
	sFrame.Format("Durée d'exposition par défaut: %i ms", DefaultDelay);
	pCmdUI->SetText ( sFrame);
}
void CMainFrame::OnUpdateFrameDelayIndicator(CCmdUI* pCmdUI)
{
	...
	sFrame.Format("Durée d'exposition de la frame: %i ms", CurrentFrameDelay);
	pCmdUI->SetText ( sFrame);
}
